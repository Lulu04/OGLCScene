...
begin
  ...
  if Sender = CheckBox1 then ScreenDemo.ShipPE.ParticlesPosRelativeToEmitterPos := CheckBox1.Checked;
end;
...
begin
  pCount := 0;
  if ScreenDemo <> NIL then pCount := ScreenDemo.ParticleCount;
  Caption := Format('OGLCScene - Scene layers   -   (%d,%d) - %d FPS  -  %d sprites  - %d particles',
  [FScene.Width, FScene.Height, FScene.FPS, FScene.SurfaceCount, pCount]);
end;
